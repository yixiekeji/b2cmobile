<template>
	<view class="returnsExchanges">
		<HeadNav nav-background-color="#f7f7f5" backImageUrl="1" textContent="我要退换货" fontSize="15" fontWeight="600"
		...
		<view class="content">
			<view class="itemGoods" v-for="item,index in ordersProductList">
				<view class="goodsContent">
					...
					<button @click="tuihuo(item.id,index)">我要退换货</button>
				</view>
				<view :class="item.id == flag?'popAct':'pop'">
					<uni-forms ref="form" :rules="rules" :modelValue="formData" label-width="80px"
						...
						<uni-forms-item :label="formData.mode == 1 ? '可换货数量':'可退货数量'">
							<view>
								{{keth}}
							</view>
						</uni-forms-item>
						<uni-forms-item label="数量" name="number" required>
							<uni-easyinput v-model="formData.number" type="number" placeholder="请输入数量"
								:styles="styles" />
						</uni-forms-item>
						<uni-forms-item label="问题描述" name="question" required v-if="item.id == flag" >
							<!-- <uni-easyinput v-model="formData.question" type="text" placeholder="请输入问题描述"
								:styles="styles" /> -->
								<uni-easyinput autoHeight :styles="stylesd" type="textarea" v-model="formData.question" placeholder="请输入问题描述"></uni-easyinput>
							<!-- <piaoyi-editor :values="formData.question" :maxlength="3000" @changes="saveContens"
								:readOnly="readOnly" :photoUrl="photoUrl" :api="api" :name="name" /> -->
						</uni-forms-item>
					</uni-forms>
					<button @click="commit(item.id,item.exchangeNumber)">提交</button>
				</view>
			</view>
		</view>
	</view>
...
<script setup>
	...
	let rules = ref({
		...
		question: {
			rules: [{
					required: true,
					errorMessage: '问题描述信息必填',
				},
				{
					minLength: 10,
					errorMessage: '最少10个字符',
				},
				{
					maxLength: 100,
					errorMessage: '最多100个字符',
				}
			]
		},
	})
	...
	const photoUrl = ref("http://img.yixiekeji.com:8080/b2cimage/")
	const api = ref('/prod-api/api/uploadFileImage')
	const name = ref('imageFile')
	...
	function saveContens(e) {
		formData.value.question = e.html
	}

	function commit(val, num) {
		maxNum.value = num
		console.log(formData.value.question);
